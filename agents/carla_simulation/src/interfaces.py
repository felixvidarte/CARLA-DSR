...
Ice.loadSlice("-I ./src/ --all ./src/Carla.ice")
import RoboCompCarla

class Fullposedata(list):
    def __init__(self, iterable=list()):
        super(Fullposedata, self).__init__(iterable)

    def append(self, item):
        assert isinstance(item, RoboCompCarla.Posedata)
        super(Fullposedata, self).append(item)

    def extend(self, iterable):
        for item in iterable:
            assert isinstance(item, RoboCompCarla.Posedata)
        super(Fullposedata, self).extend(iterable)

    def insert(self, index, item):
        assert isinstance(item, RoboCompCarla.Posedata)
        super(Fullposedata, self).insert(index, item)

setattr(RoboCompCarla, "Fullposedata", Fullposedata)
class Actors(list):
    def __init__(self, iterable=list()):
        super(Actors, self).__init__(iterable)

    def append(self, item):
        assert isinstance(item, RoboCompCarla.Actor)
        super(Actors, self).append(item)

    def extend(self, iterable):
        for item in iterable:
            assert isinstance(item, RoboCompCarla.Actor)
        super(Actors, self).extend(iterable)

    def insert(self, index, item):
        assert isinstance(item, RoboCompCarla.Actor)
        super(Actors, self).insert(index, item)

setattr(RoboCompCarla, "Actors", Actors)
class Fullresults(list):
    def __init__(self, iterable=list()):
        super(Fullresults, self).__init__(iterable)

    def append(self, item):
        assert isinstance(item, RoboCompCarla.Simresult)
        super(Fullresults, self).append(item)

    def extend(self, iterable):
        for item in iterable:
            assert isinstance(item, RoboCompCarla.Simresult)
        super(Fullresults, self).extend(iterable)

    def insert(self, index, item):
        assert isinstance(item, RoboCompCarla.Simresult)
        super(Fullresults, self).insert(index, item)

setattr(RoboCompCarla, "Fullresults", Fullresults)
...
class Implements:
    def __init__(self, ice_connector, default_handler):
        self.ice_connector = ice_connector
        self.carla = self.create_adapter("Carla", carlaI.CarlaI(default_handler))
