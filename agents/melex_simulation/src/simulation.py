...
class VehicleType:
    def __init__(self, id=None, role='actor', tx=0, ty=0, tz=0, rx=0, ry=0, rz=0):
        self.id = id
        self.role = role
        self.tx = tx
        self.ty = ty
...
class Simulation:
    ...
    def load_ego_vehicle(self, vehicle): #Esta la podemos borrar después, es para probar con un solo coche
        ...
        try:
            ...
            spawn_point = random.choice(spawn_points) if spawn_points else carla.Transform()
            print("Error to loaded vehicle")
        self._vehicles.append(self.world.try_spawn_actor(ego_bp, spawn_point))
        self.ego_vehicle = self._vehicles[0]
